...
- `publications.html` — GENERATED, do not hand-edit. Regenerated weekly from DBLP (pid 78/3993-3) by the `update-publications` GitHub Actions workflow; run `python3 tools/generate_publications.py` for a manual refresh (`--xml PATH` to use a local export). Formal publications only; CoRR preprints are filtered out. Accepted papers DBLP has not indexed yet can be listed in `tools/extra_publications.json` (auto-deduped once DBLP catches up).
